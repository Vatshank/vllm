...
# class Fp8LinearMethodRowWise(LinearMethodBase):
class Fp8LinearMethod(LinearMethodBase):
    """Linear method for FP8.
    Supports loading FP8 checkpoints with static weight scale and
    dynamic/static activation scale.
    ...
    def process_weights_after_loading(self, layer: Module) -> None:
        ...
        if not self.quant_config.is_checkpoint_fp8_serialized:
            ...
            # qweight, weight_scale = ops.scaled_fp8_quant(layer.weight,
            #                                              scale=None)
            qweight, weight_scale = torch.ops.fbgemm.quantize_fp8_per_row(layer.weight)
            # print(weight_scale)
            # Update the layer with the new values.
            # layer.weight = Parameter(qweight.t().contiguous(), requires_grad=False)
            # TODO: (vatshc) note the original implementation does a transpose here
            layer.weight = Parameter(qweight, requires_grad=False)
            # print("layer: ", layer)
            # print("layer.weight shape: ", layer.weight.shape)
            layer.weight_scale = Parameter(weight_scale, requires_grad=False)
            layer.input_scale = None
        # TODO: (vatshc) set ub constant properly
            ACTIVATION_SCALE_UB = 1200.0
            
            layer.activation_scale_ub = torch.tensor(
                [ACTIVATION_SCALE_UB],
                dtype=torch.float,
    ...
    def apply(self,
        ...
        return apply_fp8_linear(
            ...
            weight_scale=layer.weight_scale,
            input_scale=layer.input_scale,
            activation_scale_ub=layer.activation_scale_ub,
            bias=bias,
            cutlass_fp8_supported=self.cutlass_fp8_supported)
